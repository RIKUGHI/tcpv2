...
const loadData = async url => {
  ...
  res.namaPencarian == 'Semua' ? containerDataTitle.textContent = 'Daftar Satuan' : containerDataTitle.textContent = `Daftar Pencarian Untuk "${res.namaPencarian}"`
...
loadData(location.search == '' ? '?data=satuan' : location.search)

// ----------------------------------- search data satuan start -----------------------------------
const formSearch = document.querySelector('main .css-sj1k2s .css-wrap12 form')
formSearch.onsubmit = e => {
  e.preventDefault()
  if (formSearch.querySelector('input').value == '') {
    location.href = baseUrl('master/satuan')
  } else {
    location.href = baseUrl('master/satuan?data=satuan&nama='+formSearch.querySelector('input').value)
  }
}
// ----------------------------------- search data satuan end -----------------------------------

// ----------------------------------- add data satuan start -----------------------------------
addBtn.onclick = () => {
  const modalOptions = {
    type: 'main',
    title: 'Tambah Satuan',
    formGroup: [
      {
        label: 'Nama Satuan',
        warningMessage: 'Nama Satuan Wajib Diisi'
      }
    ],
    btns: [
      {
                          ...
                          ${res.products.map(r => {
                            return  `
                                      <tr>
                                        ...
                                        <td>${r.nama}</td>
                                        <td>
                                          <div class="action" data-id="${r.id_satuan}" data-nama="${r.nama}">
...
const showPagination = res => {
  ...
  if (res.halamanAktif > 1) {
    p +=  `
            <a href="${baseUrl('master/satuan')}" class="pagination-btn nb">
              <i class="fas fa-angle-double-left"></i>
            </a>
            <a href="${res.namaPencarian == 'Semua' ? baseUrl(`master/satuan?data=${res.data}&page=${res.halamanAktif - 1}`) : baseUrl(`master/satuan?data=${res.data}&nama=${res.namaPencarian}&page=${res.halamanAktif - 1}`)}" class="pagination-btn nb">
  ...
  if (res.jumlahHalaman > 5) {
    if (res.halamanAktif >= 3 && res.halamanAktif < res.jumlahHalaman - 2) {
      for (let i = res.halamanAktif - 2; i <= res.halamanAktif + 2; i++) {
        p += `<a href="${res.namaPencarian == 'Semua' ? baseUrl(`master/satuan?data=${res.data}&page=${i}`) : baseUrl(`master/satuan?data=${res.data}&nama=${res.namaPencarian}&page=${i}`)}" class="pagination-btn ${i == res.halamanAktif ? 'active' : ''}">${i}</a>`
      }
  ...
  if (res.halamanAktif < res.jumlahHalaman) {
    p +=  `
            ...
            <a href="${res.namaPencarian == 'Semua' ? baseUrl(`master/satuan?data=${res.data}&page=${res.jumlahHalaman}`) : baseUrl(`master/satuan?data=${res.data}&nama=${res.namaPencarian}&page=${res.jumlahHalaman}`)}" class="pagination-btn nb">
...
const modalEdit = async id => {
  res = await getData(baseUrl('service/getsingledata?data=satuan&id='+id))
  console.log(res);
  const modalOptions = {
    type: 'edit',
    title: 'Ubah Barang',
...
const modalDelete = async (n,id) => {
  const modalOptions = {
    type: 'delete',
    title: `Apakah Yakin Ingin Menghapus Satuan <br> <strong>${n}</strong>`,
