...
const loadData = async url => {
  ...
  res.namaPencarian == 'Semua' ? containerDataTitle.textContent = 'Daftar Supplier' : containerDataTitle.textContent = `Daftar Pencarian Untuk "${res.namaPencarian}"`
...
loadData(location.search == '' ? '?data=supplier' : location.search)

// ----------------------------------- search data supplier start -----------------------------------
const formSearch = document.querySelector('main .css-sj1k2s .css-wrap12 form')
formSearch.onsubmit = e => {
  e.preventDefault()
  if (formSearch.querySelector('input').value == '') {
    location.href = baseUrl('master/supplier')
  } else {
    location.href = baseUrl('master/supplier?data=supplier&nama='+formSearch.querySelector('input').value)
  }
}
// ----------------------------------- search data supplier end -----------------------------------

// ----------------------------------- add data supplier start -----------------------------------
addBtn.onclick = () => {
  const modalOptions = {
    type: 'main',
    ...
    formGroup: [
      {
        label: 'Nama Supplier',
        warningMessage: 'Nama Supplier Wajib Diisi'
      },{
        label: 'Kota',
                                      ...
                                      return `
                                              <div class="css-product">
                                                ...
                                                <form>
                                                  ...
                                                  ${opt.formGroup.map(d => {
                                                    if (d.label == 'Alamat') {console.log(d.value);
                                                      return  `
                          ...
                          ${res.products.map(r => {
                            return  `
                                      <tr>
                                        ...
                                        <td class="nth">${r.notelp == null ? '' : r.notelp}</td>
                                        <td class="nth">${r.nohp == null ? '' : r.nohp}</td>
                                        <td>
                                          <div class="action" data-id="${r.id_supplier}" data-nama="${r.nama}">
...
const showPagination = res => {
  ...
  if (res.halamanAktif > 1) {
    p +=  `
            <a href="${baseUrl('master/supplier')}" class="pagination-btn nb">
              <i class="fas fa-angle-double-left"></i>
            </a>
            <a href="${res.namaPencarian == 'Semua' ? baseUrl(`master/supplier?data=${res.data}&page=${res.halamanAktif - 1}`) : baseUrl(`master/supplier?data=${res.data}&nama=${res.namaPencarian}&page=${res.halamanAktif - 1}`)}" class="pagination-btn nb">
  ...
  if (res.jumlahHalaman > 5) {
    if (res.halamanAktif >= 3 && res.halamanAktif < res.jumlahHalaman - 2) {
      for (let i = res.halamanAktif - 2; i <= res.halamanAktif + 2; i++) {
        p += `<a href="${res.namaPencarian == 'Semua' ? baseUrl(`master/supplier?data=${res.data}&page=${i}`) : baseUrl(`master/supplier?data=${res.data}&nama=${res.namaPencarian}&page=${i}`)}" class="pagination-btn ${i == res.halamanAktif ? 'active' : ''}">${i}</a>`
      }
  ...
  if (res.halamanAktif < res.jumlahHalaman) {
    p +=  `
            ...
            <a href="${res.namaPencarian == 'Semua' ? baseUrl(`master/supplier?data=${res.data}&page=${res.jumlahHalaman}`) : baseUrl(`master/supplier?data=${res.data}&nama=${res.namaPencarian}&page=${res.jumlahHalaman}`)}" class="pagination-btn nb">
...
const modalDetail = async id => {
  res = await getData(baseUrl('service/getsingledata?data=supplier&id='+id))
  const modalOptions = {
    type: 'detail',
...
const modalDelete = async (n,id) => {
  const modalOptions = {
    type: 'delete',
    title: `Apakah Yakin Ingin Menghapus Supplier <br> <strong>${n}</strong>`,
